  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#ed4c50]">Catalog operations</p><h2 className="mt-1 text-3xl font-black tracking-[-0.035em]">Sản phẩm & tồn kho</h2><p className="mt-2 text-sm text-slate-500">Mỗi sản phẩm có nhiều gói, giá, kho và key riêng.</p><p className="mt-1 text-xs font-semibold text-slate-400">Trang chủ lấy tối đa 3 sản phẩm được đánh dấu <span className="text-amber-600">Nổi bật</span>, theo thứ tự trang chủ.</p></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => !saving && setModalOpen(false)} title={editing ? 'Chỉnh sửa sản phẩm' : 'Tạo sản phẩm mới'} size="xl">
        <form onSubmit={save} className="space-y-7">
          ...
          <section>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-semibold"><label className="flex items-center gap-2"><input type="checkbox" checked={draft.is_active} onChange={(event) => setDraft((value) => ({ ...value, is_active: event.target.checked }))} className="h-4 w-4 accent-[#ed4c50]" />Đang mở bán</label><label className="flex items-center gap-2"><input type="checkbox" checked={draft.is_featured} onChange={(event) => setDraft((value) => ({ ...value, is_featured: event.target.checked }))} className="h-4 w-4 accent-[#ed4c50]" />Sản phẩm nổi bật</label><label className="flex items-center gap-2"><input type="checkbox" checked={draft.has_key} onChange={(event) => setDraft((value) => ({ ...value, has_key: event.target.checked }))} className="h-4 w-4 accent-[#ed4c50]" />Bàn giao bằng key</label><label className="flex items-center gap-2 text-xs text-slate-500">Thứ tự trang chủ<input type="number" min="0" value={draft.sort_order} onChange={(event) => setDraft((value) => ({ ...value, sort_order: Math.max(0, Number(event.target.value) || 0) }))} className="h-9 w-20 rounded-lg border border-slate-200 bg-white px-2 text-center text-sm font-bold text-slate-900 outline-none focus:border-red-300 focus:ring-4 focus:ring-red-50" /></label></div>
          </section>
